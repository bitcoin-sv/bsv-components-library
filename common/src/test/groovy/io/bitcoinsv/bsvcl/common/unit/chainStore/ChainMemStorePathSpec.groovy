...
import static io.bitcoinjsv.bsvcl.common.unit.chainStore.NodeTestFactory.*
...
class ChainMemStorePathSpec extends Specification {
    ...
    def "Simple Path"() {
        ...
        io.bitcoinsv.bsvcl.common.chainStore.ChainMemStore<String, NodeTest> chain = new io.bitcoinsv.bsvcl.common.chainStore.ChainMemStore<>(genesis(), 0)
        ...
        io.bitcoinsv.bsvcl.common.chainStore.ChainPath<NodeTest> path3 = chain.getPath(0, node("3").getId(), false)
            io.bitcoinsv.bsvcl.common.chainStore.ChainPath<NodeTest> path3Expected = new io.bitcoinsv.bsvcl.common.chainStore.ChainPath<>(
                    Arrays.asList(genesis(), node("1"), node("2"), node("3")), 0)

            // Path to node3, from genesis, including descendents
            io.bitcoinsv.bsvcl.common.chainStore.ChainPath<NodeTest> path3B = chain.getPath(0, node("3").getId(), true)
            io.bitcoinsv.bsvcl.common.chainStore.ChainPath<NodeTest> path3BExpected = new io.bitcoinsv.bsvcl.common.chainStore.ChainPath<>(
                    Arrays.asList(genesis(), node("1"), node("2"), node("3"), node("4"), node("5"), node("6")), 0)

            // Path to node3, from height 2, not including descendents
            io.bitcoinsv.bsvcl.common.chainStore.ChainPath<NodeTest> path3C = chain.getPath(2, node("3").getId(), false)
            io.bitcoinsv.bsvcl.common.chainStore.ChainPath<NodeTest> path3CExpected = new io.bitcoinsv.bsvcl.common.chainStore.ChainPath<>(
                Arrays.asList(node("2"), node("3")), 0)

            // Path to node3, from height 10, not including descendents
            io.bitcoinsv.bsvcl.common.chainStore.ChainPath<NodeTest> path3D = chain.getPath(10, node("3").getId(), false)
            io.bitcoinsv.bsvcl.common.chainStore.ChainPath<NodeTest> path3DExpected = new io.bitcoinsv.bsvcl.common.chainStore.ChainPath<>(new ArrayList<io.bitcoinsv.bsvcl.common.chainStore.Node>(), 10)

            // Path to an non-existing node:
            io.bitcoinsv.bsvcl.common.chainStore.ChainPath<NodeTest> path9 = chain.getPath(0, node("9").getId(), false)
    ...
    def "multiple branches"() {
        ...
        when:
            ...
            io.bitcoinsv.bsvcl.common.chainStore.ChainPath<NodeTest> path3C = chain.getPath(0, node("3C").getId(), true)
            io.bitcoinsv.bsvcl.common.chainStore.ChainPath<NodeTest> path3CExpected = new io.bitcoinsv.bsvcl.common.chainStore.ChainPath<>(
                Arrays.asList(genesis(), node("1"), node("2"), node("3C"), node("4C"), node("5C")), 0)

            // Path to 5D, including descendents
            io.bitcoinsv.bsvcl.common.chainStore.ChainPath<NodeTest> path5D = chain.getPath(0, node("5D").getId(), true)
            io.bitcoinsv.bsvcl.common.chainStore.ChainPath<NodeTest> path5DExpected = new io.bitcoinsv.bsvcl.common.chainStore.ChainPath<>(
