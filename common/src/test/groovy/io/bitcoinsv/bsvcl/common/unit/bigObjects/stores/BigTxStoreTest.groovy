...
class BigTxStoreTest extends Specification {
    ...
    static class BlockTxSerializer implements io.bitcoinsv.bsvcl.common.bigObjects.stores.ObjectSerializer<BlockTxReceived> {
        @Override
        void serialize(BlockTxReceived object, io.bitcoinsv.bsvcl.common.bytes.ByteArrayWriter writer) {
            writer.write(new VarInt(object.getTxContent().length).encode());
            writer.write(object.getTxContent());
        }
        @Override
        BlockTxReceived deserialize(io.bitcoinsv.bsvcl.common.bytes.ByteArrayReader reader) {
            int numBytes = (int) io.bitcoinsv.bsvcl.common.serialization.BitcoinSerializerUtils.deserializeVarInt(reader);
    ...
    def "testing saving a Big Item"() {
        ...
        io.bitcoinsv.bsvcl.common.bigObjects.stores.BigCollectionChunksStore<BlockTxReceived> receiver = new io.bitcoinsv.bsvcl.common.bigObjects.stores.BigCollectionChunksStoreCMap3(
                runtimeConfig,
                STORE_ID,
        ...
        io.bitcoinsv.bsvcl.common.bigObjects.BigCollectionChunk<BlockTxReceived> chunkTx = new io.bitcoinsv.bsvcl.common.bigObjects.BigCollectionChunkImpl<>(List.of(tx), 0);
        receiver.save(HASH, chunkTx);

        // Now we iterate and return all the Txs....
        int txIndex = 0;
        Iterator<io.bitcoinsv.bsvcl.common.bigObjects.BigCollectionChunk<BlockTxReceived>> chunksReceived = receiver.getChunks(HASH);
        while (chunksReceived.hasNext()) {
            io.bitcoinsv.bsvcl.common.bigObjects.BigCollectionChunk<BlockTxReceived> chunk = chunksReceived.next();
