...
import com.nchain.jcl.base.tools.util.RuntimeUtils;
...
public class ECDSA {
    ...
    public static ECDSASignature doSign(Sha256Wrapper input, BigInteger privateKeyForSigning) {
        if (Secp256k1Context.isEnabled()) {
            try {
                byte[] signature = NativeSecp256k1.sign(
                        input.getBytes(),
                        ByteTools.bigIntegerToBytes(privateKeyForSigning, 32)
                );
