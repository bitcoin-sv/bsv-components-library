 ...
 * This avoids unneeded array copy where the BOS is simply being used to extractReader a byte array of known length from a
...
public class UnsafeByteArrayOutputStream extends ByteArrayOutputStream {
    ...
    public void write(int b) {
        ...
        if (newcount > buf.length) {
            buf = ByteTools.copyOf(buf, Math.max(buf.length << 1, newcount));
        }
        buf[count] = (byte) b;
        count = newcount;
    }
    ...
    public byte toByteArray()[] {
        return count == buf.length ? buf : ByteTools.copyOf(buf, count);
    }
