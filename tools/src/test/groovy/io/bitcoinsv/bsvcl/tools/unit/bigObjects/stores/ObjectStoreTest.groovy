...
class ObjectStoreTest extends Specification {
    ...
    class TestSerializer implements io.bitcoinsv.bsvcl.tools.bigObjects.stores.ObjectSerializer<TestClass> {
        @Override
        void serialize(TestClass object, io.bitcoinsv.bsvcl.tools.bytes.ByteArrayWriter writer) {
            writer.writeUint16LE(object.id)
            writer.writeUint32LE(object.content.length)
            writer.write(object.content);
        }
        @Override
        TestClass deserialize(io.bitcoinsv.bsvcl.tools.bytes.ByteArrayReader reader) {
    ...
    private io.bitcoinsv.bsvcl.tools.bigObjects.stores.ObjectStore<TestClass> getStore(String storeId, io.bitcoinsv.bsvcl.tools.config.RuntimeConfig runtimeConfig) {
        return new io.bitcoinsv.bsvcl.tools.bigObjects.stores.ObjectStoreCMap<TestClass>(
                runtimeConfig,
                storeId,
                new TestSerializer(),
                AVG_KEY_SIZE,
    ...
    def "testing Folder %& files"() {
        ...
        io.bitcoinsv.bsvcl.tools.bigObjects.stores.ObjectStore<TestClass> store = getStore(storeId, runtimeConfig);
        store.start()
