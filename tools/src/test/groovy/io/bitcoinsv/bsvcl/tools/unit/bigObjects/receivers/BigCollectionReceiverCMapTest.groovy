package io.bitcoinsv.bsvcl.tools.unit.bigObjects.receivers
...
class BigCollectionReceiverCMapTest extends Specification {
    ...
    private io.bitcoinsv.bsvcl.tools.bigObjects.receivers.BigCollectionReceiverCMap<ItemTest> getReceiver(io.bitcoinsv.bsvcl.tools.config.RuntimeConfig runtimeConfig, String receiverId) {
        return new io.bitcoinsv.bsvcl.tools.bigObjects.receivers.BigCollectionReceiverCMap<ItemTest>(runtimeConfig,
                receiverId, new ItemTestSerializer(), KEY_BYTES_SIZE, MAX_NUM_ENTRIES, ITEM_BYTES_SIZE, CMAP_MAX_ENTRIES);
    }

    private io.bitcoinsv.bsvcl.tools.bigObjects.BigCollectionChunk<ItemTest> createChunk(int numItems, int ordinal) {
        List<ItemTest> items = IntStream.range(0, numItems)
                .mapToObj({i -> new ItemTest(new byte[ITEM_BYTES_SIZE])})
                .collect(Collectors.toList())
        return new io.bitcoinsv.bsvcl.tools.bigObjects.BigCollectionChunkImpl<>(items, ordinal)
    }
    ...
    def "testing receiving complete Collection and callback"() {
        ...
        when:
            io.bitcoinsv.bsvcl.tools.bigObjects.receivers.BigCollectionReceiver<ItemTest> receiver = getReceiver(runtimeConfig, receiverId);
            receiver.start()
            ...
            io.bitcoinsv.bsvcl.tools.bigObjects.BigCollectionChunk<ItemTest> col1Chunk1 = createChunk(1,0)
            io.bitcoinsv.bsvcl.tools.bigObjects.BigCollectionChunk<ItemTest> col2Chunk1 = createChunk(2,0)
            io.bitcoinsv.bsvcl.tools.bigObjects.BigCollectionChunk<ItemTest> col2Chunk2 = createChunk(1,1)

            // We also send a "partial" collection:
            io.bitcoinsv.bsvcl.tools.bigObjects.BigCollectionChunk<ItemTest> col3Chunk1 = createChunk(1,1)
    ...
    def "testing changing Source and callback"() {
        ...
        when:
            ...
            io.bitcoinsv.bsvcl.tools.bigObjects.BigCollectionChunk<ItemTest> col1Chunk1 = createChunk(2,0)
            receiver.registerIncomingItems("collection-1", col1Chunk1, "source-1")
            io.bitcoinsv.bsvcl.tools.bigObjects.BigCollectionChunk<ItemTest> col1Chunk2 = createChunk(3,0)
            receiver.registerIncomingItems("collection-1", col1Chunk2, "source-2")


            // Now we get the iterator, to check the content is right:
            Iterator<io.bitcoinsv.bsvcl.tools.bigObjects.BigCollectionChunk<ItemTest>> it = receiver.getCollectionChunks("collection-1")
