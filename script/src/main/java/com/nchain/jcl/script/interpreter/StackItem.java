...
public class StackItem<C> {
    ...
    static {
        for (int i = 0; i < SMALL_NUM_STACKBYTES.length; i++) {
            ...
            if (i == 0) {
                ...
                bytes = ByteTools.reverseBytes(ByteTools.encodeMPI(BigInteger.valueOf(i), false));
            }
            SMALL_NUM_STACKBYTES[i] = ScriptData.builder().data(bytes).build();
        }
    }
    ...
    public static final StackItem forSmallNum(int num) {
        ...
        return StackItem.forBytes(SMALL_NUM_STACKBYTES[num].toBuilder().build(), StackItem.Type.INT, false);
    }
    ...
    public static StackItem wrap(byte[] bytes) {
        return new StackItem(ScriptData.builder().data(bytes).build(), Type.BYTES, false);
    }

    public static StackItem wrapDerived(byte[] bytes, boolean derived) {
        return new StackItem(ScriptData.builder().data(bytes).build(), Type.BYTES, true);
    }
    ...
    public static StackItem forBytes(byte[] bytes, Type type, boolean derived) {
        return new StackItem(ScriptData.builder().data(bytes).build(), type, derived);
    }

    public static StackItem forBytes(byte[] bytes, StackItem ... derivedFrom) {
        return new StackItem(ScriptData.builder().data(bytes).build(), Type.BYTES, false, derivedFrom);
    }
    ...
    public BigInteger getInteger() {
        return ByteTools.decodeMPI(bytes.data(), false);
    }
    ...
    public String toString() {
        ...
        if (type == StackItem.Type.STRING) {
            ...
            rendered = "0x" + HEX.encode(bytes.data());
        }
