 ...
 */
public class HandshakeHandlerImplOriginal extends HandlerImpl implements HandshakeHandler {
   ...
   private HandshakeHandlerConfig config;
    ...
    public HandshakeHandlerImplOriginal(String id, RuntimeConfig runtimeConfig, HandshakeHandlerConfig config) {
    ...
    private void processVersionMessage(HandshakePeerInfo peerInfo, BitcoinMsg<VersionMsg> message) {
        ...
        logger.debug( peerInfo.getPeerAddress(), " received VersionMsg :: " + message.getBody().toString());
        try {
            lock.lock();
            ...
            } else logger.debug( peerInfo.getPeerAddress(), " Handshake not accepted, still missing ACK from peer");
        } finally {
            lock.unlock();
        }
    }
    ...
    private void processAckMessage(HandshakePeerInfo peerInfo, BitcoinMsg<VersionAckMsg> message) {
        ...
        try {
            ...
            if (peerInfo.checkHandshakeOK()) {
                acceptHandshake(peerInfo);
            } logger.debug( peerInfo.getPeerAddress(), " Handshake not accepted, ACK not sent to Peer yet");
        } finally {
            lock.unlock();
        }
    }
    ...
    private void startHandshake(HandshakePeerInfo peerInfo) {
        ...
        System.out.println(" VERSION SEnt to " + peerInfo.getPeerAddress().toString() + ":: " + versionMsg.toString());
    }
