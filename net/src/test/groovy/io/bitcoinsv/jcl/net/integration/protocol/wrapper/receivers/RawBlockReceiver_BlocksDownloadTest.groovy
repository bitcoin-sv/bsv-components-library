...
class RawBlockReceiver_BlocksDownloadTest extends Specification {
    ...
    private static RawBlockReceiver receiver;
    ...
    private void printBlocksState(HandlerStateEvent stateEvent) {
        if (maxPeersReached.get()) {
            BlockDownloaderHandlerState blocksState = (BlockDownloaderHandlerState) stateEvent.getState()
            println(blocksState.toString())
        }
    }

    private void printPeersState(HandlerStateEvent stateEvent) {
        if (!maxPeersReached.get()) {
            HandshakeHandlerState peersState = (HandshakeHandlerState) stateEvent.getState()
    ...
    def "Testing Block Receiver"() {
        ...
        receiver = new RawBlockReceiver("testingRawBlockReceiver_blocksDownload", p2p)
