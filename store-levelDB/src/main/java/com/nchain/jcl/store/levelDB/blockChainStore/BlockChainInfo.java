 ...
 * This class is used in LevelDB to store the relative Chain info for each Block. Sicne the Blocks themselves are
 * already being stored in other keys, we only need the Block Hash here.
 *
 * If a Block is "Connected" to a Chain, then there is an entry in LevelDb for that block, storing this instane as the
 * value (for there are at least 2 entries for that block, the "regular2 one taht stores the block itself, and this one).
 *
 * NOTE: A block is "connected" to a Chain if there is an entry in LevelDb for that Block containing this info.
 */
